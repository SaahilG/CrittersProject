...
class CritterGui implements ActionListener, ChangeListener, KeyListener,
	...
	static {
		try {
			...
			if (TOURNAMENT_MODE) {
				//System.setSecurityManager(new CritterSecurityManager()); //Deprecated
			}
		} catch (Exception e) {
		}
	}
...
class CritterClassVerifier {
	...
	public static CritterGui initialSettings() {
		...
		initialValues.add((CritterModel.DEFAULT_WIDTH));
		initialValues.add((CritterModel.DEFAULT_HEIGHT));
		initialValues.add((CritterModel.DEFAULT_CRITTER_COUNT));
		if (!applet) initialValues.add((CritterGui.DEFAULT_NETWORK_ENABLED));
        initialValues.add((CritterGui.DEFAULT_DEBUG));
...
class CritterModel extends Observable implements Iterable<Critter> {
    ...
    private Object[] createRandomParameters(
        ...
        for (int j = 0; j < params.length; j++) {
            if (paramTypes[j] == Integer.TYPE) {
                ...
                params[j] = (rand.nextInt(INT_PARAM_MAX) + 1);
    ...
    private Critter fight(Critter critter1, Critter critter2, String critter1toString, String critter2toString) {
     ...
     //Added by O'Neal 2/25/2020 to prevent the winning strategy of CENTER+null
            if (weapon1 == null)
            {
                throw new BuggyCritterException("error: null weapon in class " + critter1.getClass().getName(), new NullPointerException(), critter2.getClass().getName());
            }
            else if (weapon2 == null)
            {
                throw new BuggyCritterException("error: null weapon in class " + critter2.getClass().getName(), new NullPointerException(), critter2.getClass().getName());
            }
...
class InputPane {
	...
	public static boolean showInputDialog(Frame parent, String title,
		...
		for (int i = 0; i < names.length; i++) {
			if (types[i] == Boolean.TYPE) {
				JCheckBox box = (JCheckBox) comps[i];
				values.put(names[i], (box.isSelected()));
